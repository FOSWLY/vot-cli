...
- no arguments -> `no-title`: использовать ID видео в имени файла, без попытки получить название видео

Для скриптов и интеграций добавлен аргумент `--json`. Он отключает визуальный вывод и возвращает результат обработки, `--help` или `--version` в JSON-формате.
