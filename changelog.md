...
- Были разрешены все языки для живого перевода, кроме `auto` (#72)
...
- Теперь, скрипт по умолчанию использует название видео в имени файла (если доступен [yt-dlp](https://github.com/yt-dlp/yt-dlp)). Для того, чтобы использовать ID видео в имени файла, без попытки получить название видео, был добавлен аргумент `no-title`
...
- Добавлен аргумент `json` для вывода результата, помощи и версии в JSON-формате
- Добавлен аргумент `lively-voice` для получения перевода с использованием живых голосов (#41)
- Добавлен аргумент `api-token` для устаноки Yandex OAuth token
...
- Добавлено отображение используемого рантайма при показе версии (`-v`)
- Добавлено отображение времени до завершения перевода
